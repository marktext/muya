...
export default function emoji (h, cursor, block, token, outerClass) {
  ...
  const finalClass = validation ? className : 'mu-warn'
  const CONTENT_CLASSNAME = `span.${finalClass}.mu-emoji-marked-text`
  let startMarkerCN = `span.${finalClass}.mu-emoji-marker`
