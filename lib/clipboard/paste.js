...
export default {
  async pasteHandler (event) {
    ...
    if (/html|text/.test(copyType)) {
      let markdown = copyType === 'html' && contentBlock.blockName !== 'codeblock.content'
        ? new HtmlToMarkdown({ bulletListMarker }).generate(html)
        : text

      if (/\n\n/.test(markdown) && contentBlock.blockName !== 'codeblock.content') {
        ...
        if (/html-block|math-block|diagram/.test(contentBlock.outContainer.blockName)) {
