...
export default {
  getClipboardData (event) {
    ...
    const { isSelectionInSameBlock, anchor, anchorBlock, focus, focusBlock } = this.selection.getSelection()
    ...
    const getPartialState = (position) => {
      ...
      if (/block-qupte|code-block|html-block|table|math-block|frontmatter|diagram/.test(outBlock.blockName)) {
