...
class DiagramPreview extends Parent {
  ...
  clickHandler (event) {
    ...
    this.muya.editor.activeContentBlock = cursorBlock
  ...
  async update (code = this.code) {
    ...
    if (code) {
      ...
      try {
        ...
        this.domNode.innerHTML = `<div class="mu-diagram-error">&lt; ${i18n.t('Invalid Diagram Code')} &gt;</div>`
      }
    } else {
      this.domNode.innerHTML = `<div class="mu-empty">&lt; ${i18n.t('Empty Diagram')} &gt;</div>`
    }
  }
}
