...
class Content extends TreeNode {
  ...
  set text (text) {
    ...
    this._text = text
    const { path } = this
    if (this.static.blockName === 'language-input') {
      path.pop()
      path.push('meta', 'lang')
    }
    console.log(path, oldText, text)
      ...
      if (
        ...
        if (/^delete/.test(event.inputType)) {
          ...
          if (
            ...
            (autoPairQuote && /["]{1}/.test(inputChar)) ||
            (autoPairBracket && /[\{\[\(]{1}/.test(inputChar)) ||
            (type === 'format' && !isInInlineMath && !isInInlineCode && autoPairMarkdownSyntax && /[*$`~_]{1}/.test(inputChar)))
          ) {
            needRender = true
