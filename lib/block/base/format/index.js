...
import { mixins, conflict } from '@/utils'
import { correctImageSrc } from '@/utils/image'
...
class Format extends Content {
  ...
  checkNeedRender (cursor = this.selection) {
    ...
    for (const token of tokenizer(text, {
      ...
      if (
        conflict([Math.max(0, start - 1), Math.min(textLen, end + 1)], [anchorOffset, anchorOffset]) ||
        conflict([Math.max(0, start - 1), Math.min(textLen, end + 1)], [focusOffset, focusOffset])
      ) {
        return true
      }
    }
