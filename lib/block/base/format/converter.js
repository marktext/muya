...
export default {
  convertIfNeeded () {
    ...
    console.log(match, atxHeading, setextHeading, blockquote, thematicBreak)
    switch (true) {
  ...
  convertToThematicBreak () {
    ...
    if (hasFocus) {
      ...
      const cursor = {
        path: thematicBreakContent.path,
        block: thematicBreakContent,
        start: { offset: startOffset },
        end: { offset: endOffset }
      }

      thematicBreakContent.update(cursor)
  ...
  convertToAtxHeading (atxHeading) {
    if (this.parent.static.blockName === 'atx-heading') {
      return
    }
    ...
    if (hasFocus) {
      ...
      const endOffset = Math.max(0, end.offset - preParagraphTextLength)
      const cursor = {
        path: atxHeadingContent.path,
        block: atxHeadingContent,
        start: { offset: startOffset },
        end: { offset: endOffset }
      }

      atxHeadingContent.update(cursor)
      this.selection.setSelection(cursor)
    }
  ...
  convertToSetextHeading (setextHeading) {
    if (this.parent.static.blockName === 'setext-heading') {
      return
    }
    ...
    if (hasFocus) {
      ...
      const cursor = {
        ...
        start: {
          offset: cursorBlock.text.length
        },
        end: {
          offset: cursorBlock.text.length
        }
  ...
  convertToBlockQuote () {
    ...
    if (hasFocus) {
      ...
      const cursor = {
        block: cursorBlock,
        path: cursorBlock.path,
        start: { offset: Math.max(0, start.offset - 1) },
        end: { offset: Math.max(0, end.offset - 1) }
      }
      cursorBlock.update(cursor)
      this.selection.setSelection(cursor)
    }
  ...
  convertToParagraph () {
    if (
      this.parent.static.blockNode === 'setext-heading' ||
