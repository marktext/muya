...
export default {
  ...
  unindentListItem (type) {
    ...
    if (type === 'REPLACEMENT') {
      ...
      if (listItem.next && newListItem.children.tail.blockName !== list.blockName) {
        const state = {
          name: list.blockName,
          meta: { ...list.meta },
          children: []
      ...
      if (listItem.next) {
        ...
        list.forEachAt(offset + 1, undefined, node => {
          newListItem.children.tail.append(node.clone(), 'user')
          node.remove()
        })
      }
      ...
      newListItem.firstContentInDescendant().setCursor(start.offset, end.offset, true)
    }
  },
  ...
  indentListItem () {
    ...
    let newList = prevListItem?.children?.tail
    ...
    newList.firstContentInDescendant().setCursor(start.offset, end.offset, true)
  },
