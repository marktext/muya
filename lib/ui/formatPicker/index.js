...
class FormatPicker extends BaseFloat {
  ...
  listen () {
    ...
    const handleKeyup = event => {
      ...
      if (activeElement.classList.contains('mu-content')) {
        const block = activeElement[BLOCK_DOM_PROPERTY]
        if (!(block instanceof Format) || !metaKey) {
          return
        }
        if (shiftKey) {
          if (Object.keys(SHIFT_HASH).includes(key)) {
            event.preventDefault()
            block.format(SHIFT_HASH[key])
          }
        } else {
          if (Object.keys(HASH).includes(key)) {
            event.preventDefault()
            block.format(HASH[key])
          }
        }
      }
    }

    eventCenter.attachDOMEvent(document, 'keydown', handleKeyup)
  }
