...
class FrontButton {
  ...
  listen () {
    ...
    const mousemoveHandler = throttle(event => {
      ...
      const { x, y } = event
      const eles = [...document.elementsFromPoint(x, y), ...document.elementsFromPoint(x + 50, y)]
  ...
  mouseMove = (event) => {
    ...
    if (outMostElement && outMostElement[BLOCK_DOM_PROPERTY] !== this.dragInfo.block) {
  ...
  startDrag = () => {
    ...
    this.dragEvents = [
      eventCenter.attachDOMEvent(document, 'mousemove', throttle(this.mouseMove)),
