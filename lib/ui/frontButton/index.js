...
const renderIcon = (i, className) => h(`i.icon${className ? `.${className}` : ''}`, h('i.icon-inner', {
  style: {
...
class FrontButton {
  ...
  mouseUp = (event) => {
    ...
    if (target && position) {
      ...
      const { anchorBlock, anchor, focus, isSelectionInSameBlock } = block.muya.editor.selection ?? {}
