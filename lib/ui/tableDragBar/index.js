...
class TableDragBar extends BaseFloat {
  ...
  listen () {
    ...
    const handler = throttle(event => {
      ...
      if (!this.isDragTableBar && !hasTableCell(eles) && (hasTableCell(aboveEles) || hasTableCell(leftEles))) {
        const tableCellEle = [...aboveEles, ...leftEles].find(ele => ele[BLOCK_DOM_PROPERTY] && ele[BLOCK_DOM_PROPERTY].blockName === 'table.cell')
  ...
  switchTableData = () => {
    ...
    if (table.active) {
      const { anchorBlock, anchor, focus, isSelectionInSameBlock } = this.muya.editor.selection ?? {}
      const { rowOffset, columnOffset } = anchorBlock.closestBlock('table.cell')
