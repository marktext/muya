...
export const getIndex = (barType, cellBlock) => {
  ...
  return barType === 'top' ? row.offset(cellBlock) : table.firstChild.offset(row)
}
...
class TableDragBar extends BaseFloat {
  ...
  constructor (muya, options = {}) {
    ...
    const opts = Object.assign({}, topOptions, options)
  ...
  listen () {
    ...
    const handler = throttle(event => {
      ...
      const hasTableCell = eles => eles.some(ele => ele[BLOCK_DOM_PROPERTY] && ele[BLOCK_DOM_PROPERTY].blockName === 'table.cell')

      if (!this.isDragTableBar && !hasTableCell(eles) && (hasTableCell(bellowEles) || hasTableCell(leftEles))) {
        const tableCellEle = [...bellowEles, ...leftEles].find(ele => ele[BLOCK_DOM_PROPERTY] && ele[BLOCK_DOM_PROPERTY].blockName === 'table.cell')
        const cellBlock = tableCellEle[BLOCK_DOM_PROPERTY]
        const barType = hasTableCell(bellowEles) ? 'top' : 'right'

        this.options = Object.assign({}, barType === 'right' ? rightOptions : topOptions)
  ...
  docMousemove = (event) => {
    ...
    const attrName = barType === 'top' ? 'clientX' : 'clientY'
  ...
  setDragTargetStyle = () => {
    ...
    for (const cell of dragCells) {
      ...
      const valueName = barType === 'top' ? 'translateX' : 'translateY'
