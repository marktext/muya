...
export const getLabelFromEvent = event => {
  ...
  const result = ALL_MENU_CONFIG.find(menu => {
    ...
    return code === shortKeyMap.code && metaKey === shortKeyMap.metaKey && shiftKey === shortKeyMap.shiftKey && altKey === shortKeyMap.altKey
  })
...
export const replaceBlockByLabel = ({ block, muya, label, text = '' }) => {
  ...
  muya.editor.activeContentBlock = cursorBlock
}
