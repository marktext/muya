...
const parser = (function () {
  ...
  return (
    ((parser as any).lexer = lexer),
    (Parser.prototype = parser),
    ((parser as any).Parser = Parser),
    new Parser()
  );
})();
...
(Diagram as any).ParseError = ParseError;
...
function wobble(x1, y1, x2, y2) {
  ...
  const xfactor = Math.random() > 0.5 ? factor : -factor;
  const yfactor = Math.random() > 0.5 ? factor : -factor;
...
_.extend(BaseTheme.prototype, {
  ...
  draw: function (container) {
    ...
    const titleHeight = this.title_ ? this.title_.height : 0;
    const y = DIAGRAM_MARGIN + titleHeight;
  ...
  drawSelfSignal: function (signal, offsetY) {
    ...
    const textBB = signal.textBB;
    const aX = getCenterX(signal.actorA);

    const x = aX + SELF_SIGNAL_WIDTH + SIGNAL_PADDING;
    const y = offsetY + SIGNAL_PADDING + signal.height / 2 + textBB.y;

    this.drawText(x, y, signal.message, this.font_, ALIGN_LEFT);

    const y1 = offsetY + SIGNAL_MARGIN + SIGNAL_PADDING;
    const y2 = y1 + signal.height - 2 * SIGNAL_MARGIN - SIGNAL_PADDING;
  ...
  drawNote: function (note, offsetY) {
    note.y = offsetY;
    const actorA = note.hasManyActors() ? note.actor[0] : note.actor;
    const aX = getCenterX(actorA);
...
if (typeof Snap !== "undefined") {
  ...
  _.extend(SnapTheme.prototype, BaseTheme.prototype, {
    ...
    finishGroup: function () {
      const g = this.paper_.group.apply(this.paper_, this._stack);
    ...
    createText: function (text, font) {
      ...
      const t = this.paper_.text(0, 0, text);
      t.attr(font || {});
      if (text.length > 1) {
    ...
    drawLine: function (x1, y1, x2, y2, linetype, arrowhead) {
      const line = this.paper_.line(x1, y1, x2, y2).attr(LINE);
    ...
    drawRect: function (x, y, w, h) {
      const rect = this.paper_.rect(x, y, w, h).attr(RECT);
  ...
  _.extend(SnapHandTheme.prototype, SnapTheme.prototype, {
    drawLine: function (x1, y1, x2, y2, linetype, arrowhead) {
      const line = this.paper_.path(handLine(x1, y1, x2, y2)).attr(LINE);
      if (linetype !== undefined) {
        line.attr("strokeDasharray", this.lineTypes_[linetype]);
      }
    ...
    drawRect: function (x, y, w, h) {
      const rect = this.paper_.path(handRect(x, y, w, h)).attr(RECT);
...
if (typeof Snap === "undefined") {
  throw new Error("Snap.svg is required to be included.");
}
...
Diagram.prototype.drawSVG = function (container, options) {
  ...
  const Theme = Diagram.themes[options.theme];
